Feature: Calendar searching
  ...
  Scenario: Test complex calendar daily opening information
    ...
    * def createCalendarRequest = read('classpath:helios/mod-calendar/features/samples/createComplexCalendar.json')
    ...
    When method GET
    Then status 200
    And match $ == read('classpath:helios/mod-calendar/features/samples/dailyOpeningsWithClosuresComplex.json')
    ...
    When method GET
    Then status 200
    And match $ == read('classpath:helios/mod-calendar/features/samples/dailyOpeningsWithoutClosuresComplex.json')
