...
class CheckOutSimulation extends Simulation {
  ...
  val before = scenario("before")
    .exec(karateFeature("classpath:vega/mod-circulation/circulation-junit.feature"))
  val create = scenario("create")
    .repeat(10) {
      exec(karateFeature("classpath:vega/mod-circulation/features/loans.feature@SuccessfulCheckOut"))
    }
