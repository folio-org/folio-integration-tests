...
#    And match accounts[1].paymentStatus.name == 'Suspended claim returned'
  ...
  Scenario: When an existing loan is aged to lost update agedToLostDate, item status to Aged to lost
    ...
    * def extLoanDate = checkOutResponse.response.loanDate

    # change due date of the loan so that it will be age to lost
    * def extNewDueDate = call read('classpath:vega/mod-circulation/features/util/get-time-now-function.js')
#    * def updateLoanRequest = { id: #(extLoanId), itemId: #(extItemId), dueDate: #(extNewDueDate), loanDate: #(extLoanDate), action: 'dueDateChanged' }
    * def updateLoanRequest = { dueDate: #(extNewDueDate) }
    Given path 'circulation', 'loans', extLoanId, 'change-due-date'
    And request updateLoanRequest
    When method POST
    Then status 204

    # trigger age to lost process
