Feature: Sender - message delivery
  ...
  Scenario: Should return 400 when user is not found
    ...
    * def deliveryChannel = 'email'
    And request read('classpath:vega/mod-sender/eureka-features/samples/message-delivery.json')
    When method POST
    Then status 400
  ...
  Scenario: Should fail when notification contains additional properties
    ...
    * def barcode = '12345678'
    And request read('classpath:vega/mod-sender/eureka-features/samples/create-recipient.json')
    When method POST
    Then status 201

    Given path 'message-delivery'
    And request read('classpath:vega/mod-sender/eureka-features/samples/message-delivery-additional-property.json')
    When method POST
    Then status 422
