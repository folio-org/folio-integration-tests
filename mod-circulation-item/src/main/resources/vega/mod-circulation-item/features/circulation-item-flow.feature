Feature: creating/updating item Scenarios
  ...
  Scenario: Validate locationId exists when creating item with effectiveLocationId
    ...
    * def locationData = call read('classpath:vega/mod-circulation-item/reusable/pre-requisites.feature@CreateDummyLocation')
    * def effectiveLocationId = locationData.response.id

    * def circulationItemId = call uuid
    * def barcode = call random_string
    * def circulationItemRequest = read('classpath:vega/mod-circulation-item/features/samples/create-dcb-circulation-item.json')
