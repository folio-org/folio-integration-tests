...
Feature: Mediated requests - create and retrieve via mod-requests-mediated
  ...
  Scenario: create and confirm item-level mediated page request
    ...
    * def inventory = inv.inventory
    ...
    # Wait for mod-search to index all consortium copies of the instance (central + university + college).
    ...
    * def headersConsortiumSearch = { 'Content-Type': 'application/json', 'Accept': 'application/json', 'x-okapi-token': '#(centralOkapitoken)', 'x-okapi-tenant': '#(centralTenant)', 'x-okapi-consortium-tenant': 'true', 'x-consortium-id': '#(consortiumId)' }
    * configure headers = headersConsortiumSearch
    * configure retry = { count: 40, interval: 15000 }
    Given path 'search/instances'
    And param query = 'id==' + inventory.instanceId
