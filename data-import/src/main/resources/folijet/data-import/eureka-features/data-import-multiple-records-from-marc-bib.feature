Feature: Util feature to import multiple entities from one incoming marc bib. Based on FAT-4834 scenario steps.
  ...
  Scenario: Import Holdings, Items. Based on FAT-4834 scenario steps.
    ...
    * def result = call read('classpath:folijet/data-import/eureka-global/common-data-import.feature') {headersUser: '#(headersUser)', headersUserOctetStream: '#(headersUserOctetStream)', uiKey : '#(uiKey)', fileName: '#(fileName)', 'filePathFromSourceRoot' : 'classpath:folijet/data-import/samples/mrc-files/MultipleHold_AndItems.mrc'}
    ...
    * call read('classpath:folijet/data-import/eureka-features/get-completed-job-execution-for-key.feature@getJobWhenJobStatusCompleted') { key: '#(result.s3UploadKey)'}
    ...
    * call pause 10000
    ...
    And headers headersUser
    And retry until response.entries[0].relatedHoldingsInfo[0].actionStatus != null && response.entries[0].relatedItemInfo[0].actionStatus != null
    When method GET
    Then status 200
    ...
    * def record = karate.filter(response.holdingsRecords, function(x){ return x.permanentLocationId == 'fcd64ce1-6995-48f0-840e-89ffa2288371'; })
    * def holdingsId = record[0].id
    * def holdingsSourceId = record[0].sourceId
