Feature: Get job execution by S3 key with retries (maps to job execution)
  ...
  Scenario: wait until job status will be 'completed'
    ...
    * def jobExecution = response
    ...
    #This scenario below accepts job profile id to filter the initial response further
  @getJobByJobProfileId
  Scenario: wait until job status will be 'completed'
    ...
    And param sortBy = 'started_date,desc'
    And param subordinationTypeNotAny = ['COMPOSITE_CHILD', 'PARENT_SINGLE']
    And param profileIdAny = __arg.jobProfileId
    And headers headersUser
    And retry until karate.get('response.jobExecutions[0].status') == 'COMMITTED' && karate.get('response.jobExecutions[0].uiStatus') == 'RUNNING_COMPLETE'
    When method get
    Then status 200

    * def parentJobExecutionId = response.jobExecutions.find(exec => exec.sourcePath == __arg.key).id
