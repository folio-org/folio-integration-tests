Feature: Util feature to import instance, holding, item. Based on FAT-937 scenario steps.
  ...
  Scenario: Import Instance, Holdings, Items. Based on FAT-937 scenario steps.
    ...
    * def filePathFromSourceRootName = 'classpath:promin/data-import/samples/mrc-files/' + fileName
    * def uiKey = fileName + randomNumber

    * print 'Before Forwarding: ', 'uiKey: ', uiKey, 'fileName: ', fileName, 'filePathFromSourceRoot: ', filePathFromSourceRootName, 'testIdentifier:', testIdentifier
    * def result = call read('classpath:promin/data-import/global/common-data-import.feature') {headersUser: '#(headersUser)', headersUserOctetStream: '#(headersUserOctetStream)', uiKey : '#(uiKey)', fileName: '#(fileName)', 'filePathFromSourceRoot' : '#(filePathFromSourceRootName)'}
