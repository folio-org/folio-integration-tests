Feature: Util feature to import multiple entities from one incoming marc bib. Based on FAT-4834 scenario steps.
  ...
  Scenario: Import Holdings, Items. Based on FAT-4834 scenario steps.
    ...
    * def result = call read('classpath:promin/data-import/global/common-data-import.feature') {headersUser: '#(headersUser)', headersUserOctetStream: '#(headersUserOctetStream)', uiKey : '#(uiKey)', fileName: '#(fileName)', 'filePathFromSourceRoot' : 'classpath:promin/data-import/samples/mrc-files/MultipleHold_AndItems.mrc'}
    ...
    * call read('classpath:promin/data-import/global/get-completed-job-execution-for-key.feature@getJobWhenJobStatusCompleted') { key: '#(result.s3UploadKey)'}
