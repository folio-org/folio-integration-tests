  Feature: Default mapping rules update and verification via data-import
  ...
  Scenario: Default mapping rules updating and verification via data-import
    * print 'Default mapping rules updating and verification via data-import'
    ...
    * def result = call read(commonImportFeature) {headersUser: '#(headersUser)', headersUserOctetStream: '#(headersUserOctetStream)', uiKey : '#(uiKey)', fileName: '#(fileName)', 'filePathFromSourceRoot' : 'classpath:promin/data-import/samples/mrc-files/FAT-1117.mrc'}
    ...
    * def instanceHrid = response.externalIdsHolder.instanceHrid
    ...
    And headers headersUser
    And request read('classpath:promin/data-import/samples/samples_for_upload/FAT-1117-changed-marc-bib-rules.json')
    When method PUT
    Then status 200
    * call pause 5000
