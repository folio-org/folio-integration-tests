...
Feature: Test matching by POL number and vendor reference number
  ...
  Scenario: FAT-2184 Match on POL and update related Instance, Holdings, Item
    # Create order
    * def result = call read('classpath:promin/data-import/global/create-order.feature') {poNumber: 'FAT2184pref10000', vendorId: 'c6dace5d-4574-411e-8ba1-036102fcdc9b'}
    ...
    * def orderLineParams =
    ...
    """
    * call read('classpath:promin/data-import/global/create-order-line.feature') orderLineParams
    ...
    * def filePath = 'classpath:promin/data-import/samples/mrc-files/' + fileName
    * def uiKey = fileName + randomNumber
    * def result = call read('classpath:promin/data-import/global/common-data-import.feature') {headersUser: '#(headersUser)', headersUserOctetStream: '#(headersUserOctetStream)', uiKey: '#(uiKey)', fileName: '#(fileName)', 'filePathFromSourceRoot': '#(filePath)'}
    ...
    * call read('classpath:promin/data-import/global/get-completed-job-execution-for-key.feature@getJobWhenJobStatusCompleted') {key: '#(sourcePath)'}
  ...
  Scenario: FAT-2183 Match on VRN and update related Instance, Holdings, Item
    # Create order
    * def result = call read('classpath:promin/data-import/global/create-order.feature') {poNumber: 'FAT2183pref10000', vendorId: 'c6dace5d-4574-411e-8ba1-036102fcdc9b'}
    * def createdOrder = result.response
    ...
    * def orderLineParams =
    ...
    {
      "orderId": "#(createdOrder.id)",
      "title": "Georgia, 1732-1742 / Jay Jordan Butler"
    }
    """
    * call read('classpath:promin/data-import/global/create-order-line-with-vrn.feature') orderLineParams
    ...
    {
      "profile": {
        "name": "FAT-2183: Update Instance by VRN match",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "existingRecordType": "INSTANCE",
        "description": "FAT-2183: Update Instance by VRN match",
        "mappingDetails": {
          "name": "instance",
          "recordType": "INSTANCE",
          "mappingFields": [
    ...
    * def updateInstanceMappingProfileId = $.id
    ...
    {
      "profile": {
        "name": "FAT-2183: Update Holdings by VRN match",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "existingRecordType": "HOLDINGS",
        "description": "FAT-2183: Update Holdings by VRN match",
        "mappingDetails": {
          "name": "holdings",
          "recordType": "HOLDINGS",
          "mappingFields": [
    ...
    * def updateHoldingsMappingProfileId = $.id
    ...
    {
      "profile": {
        "name": "FAT-2183: Update Item by VRN match",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "existingRecordType": "ITEM",
        "description": "FAT-2183: Update Item by VRN match",
        "mappingDetails": {
          "name": "item",
          "recordType": "ITEM",
          "mappingFields": [
    ...
    * def updateItemMappingProfileId = $.id
    ...
    # Create action profile for instance update
    * def folioRecordNameAndDescription = 'FAT-2183: Update Instance by VRN match'
    * def folioRecord = 'INSTANCE'
    * def profileAction = 'UPDATE'
    * def mappingProfileEntityId = updateInstanceMappingProfileId
    Given path 'data-import-profiles/actionProfiles'
    ...
    * def updateInstanceActionProfileId = $.id
    ...
    # Create action profile for holdings update
    * def folioRecordNameAndDescription = 'FAT-2183: Update Holdings by VRN match'
    * def folioRecord = 'HOLDINGS'
    * def profileAction = 'UPDATE'
    * def mappingProfileEntityId = updateHoldingsMappingProfileId
    Given path 'data-import-profiles/actionProfiles'
    ...
    * def updateHoldingsActionProfileId = $.id
    ...
    # Create action profile for item update
    * def folioRecordNameAndDescription = 'FAT-2183: Update Item by VRN match'
    * def folioRecord = 'ITEM'
    * def profileAction = 'UPDATE'
    * def mappingProfileEntityId = updateItemMappingProfileId
    Given path 'data-import-profiles/actionProfiles'
    ...
    * def updateItemActionProfileId = $.id
    ...
    # Create match profile for MARC-to-INSTANCE 024$a to vendorReferenceNumber
    Given path 'data-import-profiles/matchProfiles'
    And headers headersUser
    And request
    """
    {
      "profile": {
        "name": "FAT-2183: 024 $a VRN to Instance VRN",
        "description": "FAT-2183: 024 $a VRN to Instance VRN",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "matchDetails": [
          {
            "incomingRecordType": "MARC_BIBLIOGRAPHIC",
    ...
    * def instanceMatchProfileId = $.id
    ...
    # Create match profile for MARC-to-HOLDINGS 024$a to vendorReferenceNumber
    Given path 'data-import-profiles/matchProfiles'
    And headers headersUser
    And request
    """
    {
      "profile": {
        "name": "FAT-2183: 024 $a VRN to Holdings VRN",
        "description": "FAT-2183: 024 $a VRN to Holdings VRN",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "matchDetails": [
          {
            "incomingRecordType": "MARC_BIBLIOGRAPHIC",
    ...
    * def holdingsMatchProfileId = $.id
    ...
    # Create match profile for MARC-to-ITEM 024$a to vendorReferenceNumber
    Given path 'data-import-profiles/matchProfiles'
    And headers headersUser
    And request
    """
    {
      "profile": {
        "name": "FAT-2183: 024 $a VRN to Item VRN",
        "description": "FAT-2183: 024 $a VRN to Item VRN",
        "incomingRecordType": "MARC_BIBLIOGRAPHIC",
        "matchDetails": [
          {
            "incomingRecordType": "MARC_BIBLIOGRAPHIC",
    ...
    * def itemMatchProfileId = $.id
    ...
    # Create job profile - Update Instance, Holdings, Item based on VRN match
    Given path 'data-import-profiles/jobProfiles'
    And headers headersUser
    And request
    """
    {
      "profile": {
        "name": "FAT-2183: Update Instance, Holdings, Item based on VRN match",
        "description": "FAT-2183: Update Instance, Holdings, Item based on VRN match",
        "dataType": "MARC"
      },
      "addedRelations": [
        {
    ...
    * def jobProfileName = $.profile.name
    ...
    # Create file definition for FAT-2183.mrc and upload the file
    * def randomNumber = callonce random
  ...
  Scenario: FAT-13844 Match on POL and VRN and update related Instance
    # Create order
    * def result = call read('classpath:promin/data-import/global/create-order.feature') {poNumber: 'FAT13844pref1000', vendorId: 'c6dace5d-4574-411e-8ba1-036102fcdc9b'}
    * def createdOrder = result.response
