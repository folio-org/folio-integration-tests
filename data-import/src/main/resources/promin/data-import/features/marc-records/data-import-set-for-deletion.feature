Feature: Set for deletion logic
  ...
  Scenario: Update instance using marc with deleted leader
    ...
    * def marcRecord = read('classpath:promin/data-import/samples/mrc-files/marcBibDeletedLeader.mrc')
  ...
  Scenario: Unmark deleted instance
    ...
    * def filePathFromSourceRoot = 'file:target/' + fileName + '.mrc'
    * def marcRecord = read('classpath:promin/data-import/samples/mrc-files/marcBib.mrc')
    * def updatedMarcRecord = javaWriteData.modifyMarcRecord(marcRecord, '001', ' ', ' ', ' ', instanceHrid)
  ...
  Scenario: MODSOURCE-898 - Update deleted instance using MARC-MARC matching
    ...
    * def marcBibJson = read('classpath:promin/data-import/samples/marcBib.mrc.json')
    ...
    * field245['245'].subfields.find(sf => sf.a).a = 'Summerland_MODSOURCE-898' + timestamp + ' /'
    Given call read('classpath:promin/data-import/global/import-marc-json-and-verify.feature') { marcJsonObject: '#(marcBibJson)', fileName: 'marcBib', jobName: 'createInstance', actionStatus: 'CREATED' }
    ...
    * def dataExportMappingProfile = read('classpath:promin/data-import/samples/profiles/data-export-mapping-profile.json')
    ...
    * def result = call read(exportRecordFeature) { instanceId: "#(instanceId)", dataExportJobProfileId: "#(dataExportJobProfileId)", fileName: "#(fileName)" }
    ...
    Given call read('classpath:promin/data-import/global/setup-marc-to-marc-update-profile.feature') { profileName: 'MODSOURCE-898 MARC-MARC Update deleted', actionType: 'UPDATE', mappingOption: 'UPDATE' }
    * def jobProfileId = updateJobProfileId
