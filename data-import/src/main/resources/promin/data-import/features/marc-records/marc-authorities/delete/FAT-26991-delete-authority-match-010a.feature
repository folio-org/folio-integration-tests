@parallel=false
Feature: FAT-26991 Delete MARC Authority records matched by 010 $a
  ...
  # match profile on 010 $a. The imported file carries three records: one authority that is linked to
  ...
  @C1504477
  Scenario: Delete MARC Authority matched by 010 $a
    ...
    * def profiles = call read(commonFeature + '@CreateDeleteJobProfile') { runId: '#(runId)', profileName: 'FAT-26991 delete authority by 010 a', matchField: '010', matchSubfield: 'a', ind1: ' ', ind2: ' ' }
    ...
    * def exportedFile = exported.exportedBinaryMarcRecord
    ...
    # An 010 $a that no authority in the tenant carries
    * def deleteFile = javaWriteData.setFieldValueByControlNumber(exportedFile, nonMatchControlNumber, '010', 'a', 'fat26991x' + runId)
