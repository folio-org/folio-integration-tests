...
class GobiSimulation extends Simulation {
  ...
  val protocol = karateProtocol(
    ...
    "/gobi/orders/custom-mappings" -> Nil,
    "/gobi/orders/custom-mappings/UnlistedPrintSerial" -> Nil,
  )
  protocol.runner.systemProperty("testTenant", generateTenantId())

  val before = scenario("before")
    .exec(karateFeature("classpath:thunderjet/mod-gobi/gobi-junit.feature"))
  val create = scenario("create")
    .repeat(10) {
      exec(karateFeature("classpath:thunderjet/mod-gobi/gobi.feature"))
    }
