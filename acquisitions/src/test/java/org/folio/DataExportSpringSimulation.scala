...
class DataExportSpringSimulation extends Simulation {
  ...
  val protocol = karateProtocol(
    ...
    "/configurations/entries" -> Nil,
    "/data-export-spring/configs" -> Nil,
    "/data-export-spring/jobs" -> Nil,
  )
  protocol.runner.systemProperty("testTenant", generateTenantId())

  val before = scenario("before")
    .exec(karateFeature("classpath:thunderjet/mod-data-export-spring/data-export-spring-junit.feature"))
  val create = scenario("create")
    .repeat(10) {
      exec(karateFeature("classpath:thunderjet/mod-data-export-spring/features/root.feature"))
    }
