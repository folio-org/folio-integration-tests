Feature: open-order-multy-location-mixed-and-manual-piece-false-and-create-inventory-instance-holdings-items.
  ...
  Scenario: Check that instances, items, pieces, holdings were created
    ...
    * def itemElec2 = karate.jsonPath(response, "$.items[*][?(@.materialType.name == 'Elec')]")[1]
    ...
    * def itemPhys2 = karate.jsonPath(response, "$.items[*][?(@.materialType.name == 'Phys')]")[1]
    ...
    And match ([itemElec1.effectiveLocation.id, itemElec2.effectiveLocation.id]) contains any ['#(globalLocationsId)', '#(globalLocationsId2)']
    And match ([itemElec1.holdingsRecordId, itemElec2.holdingsRecordId]) contains any ['#(poLineHoldingId)', '#(poLineHoldingId2)']
    And match ([itemPhys1.effectiveLocation.id, itemPhys2.effectiveLocation.id]) contains any ['#(globalLocationsId)', '#(globalLocationsId2)']
    And match ([itemPhys1.holdingsRecordId, itemPhys2.holdingsRecordId]) contains any ['#(poLineHoldingId)', '#(poLineHoldingId2)']
