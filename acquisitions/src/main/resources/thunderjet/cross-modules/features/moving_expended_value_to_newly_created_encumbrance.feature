...
Feature: Moving expended amount when editing fund distribution for POL
  ...
  Scenario Outline: Prepare finances
    ...
    * def v = call createBudget { id: '#(budgetId)', fundId: '#(fundId)', fiscalYearId: '#(fyId1)', allocated: 1000 }
  ...
  Scenario: Create a invoice line linked to the po line
    * def v = call createInvoiceLineFromPoLine { invoiceLineId: '#(invoiceLineId)', invoiceId: '#(invoiceId)', poLineId: '#(poLineId)', fundId: '#(fundId1)', total: 1, releaseEncumbrance: false }
