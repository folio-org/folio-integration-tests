...
Feature: Encumbrance Calculated Correctly After Canceling Invoice Exceeding Initial Encumbrance
  ...
  Scenario: Encumbrance Calculated Correctly After Canceling Invoice Exceeding Initial Encumbrance
    ...
    * print '6. Verify Order State After Both Invoices Are Paid'
    ...
    And retry until response.workflowStatus == 'Open' && response.totalEstimatedPrice == 100.00 && response.totalEncumbered == 0.00 && response.totalExpended == 160.00 && response.totalCredited == 0.00
    When method GET
    Then status 200
