# For MODORDERS-1363, MODORDERS-1367, https://foliotest.testrail.io/index.php?/cases/view/877084
Feature: Encumbrance Is Created As Released After Manual Release And Fund Change With Paid Invoice Release False
  ...
  Scenario: Encumbrance Is Created As Released After Manual Release And Fund Change With Paid Invoice Release False
    ...
    * def v = call openOrder { orderId: "#(orderId)" }
    ...
    # 5. Create Invoice With $9 Amount And Release Encumbrance False
    * print '5. Create Invoice With $9 Amount And Release Encumbrance False'
    * def v = call createInvoice { id: "#(invoiceId)", fiscalYearId: "#(globalFiscalYearId)" }
    * def v = call createInvoiceLine { invoiceLineId: "#(invoiceLineId)", invoiceId: "#(invoiceId)", poLineId: "#(orderLineId)", fundId: "#(fundAId)", total: 9.00, releaseEncumbrance: false }
    ...
    * def v = call payInvoice { invoiceId: "#(invoiceId)" }
    ...
    # 7. Verify Order State - $1 Encumbered, $9 Expended
    * print '7. Verify Order State - $1 Encumbered, $9 Expended'
    Given path 'orders/composite-orders', orderId
    And retry until response.workflowStatus == 'Open' && response.totalEstimatedPrice == 10.00 && response.totalEncumbered == 1.00 && response.totalExpended == 9.00
    When method GET
    Then status 200

    # 8. Verify Encumbrance Is Unreleased With $1 Amount From Fund A
    * print '8. Verify Encumbrance Is Unreleased With $1 Amount From Fund A'
    * def validateEncumbranceUnreleased =
    """
    function(response) {
      var transaction = response.transactions[0];
      return transaction.amount == 1.00 &&
    ...
    * def encumbranceId = response.transactions[0].id
    ...
    # 9. Release Encumbrance Manually
    * print '9. Release Encumbrance Manually'
    Given path 'finance/release-encumbrance', encumbranceId
    And request {}
    When method POST
    Then status 204

    # 10. Verify Encumbrance Status Is Released With $0 Amount
    * print '10. Verify Encumbrance Status Is Released With $0 Amount'
    * def validateEncumbranceReleased =
    ...
    # 11. Change Fund Distribution From Fund A To Fund B In The Order Line
    * print '11. Change Fund Distribution From Fund A To Fund B In The Order Line'
    ...
    * remove poLine.fundDistribution[0].encumbrance
    ...
    # 12. Verify New Encumbrance Is Created As Released With $0 Amount From Fund B
    * print '12. Verify New Encumbrance Is Created As Released With $0 Amount From Fund B'
