Feature: Consortia Sharing Instances api tests
  ...
  Scenario: Attempt to POST a sharingSetting with invalid request body or non-existing path id
    ...
    When method POST
    Then status 422
    And match response == { errors: [{message: "'settingId' validation failed. must not be null", type: '-1', code: 'sharingSettingValidationError'}] }
    ...
    When method POST
    Then status 422
    And match response == { errors: [{message: "'url' validation failed. must not be null", type: '-1', code: 'sharingSettingValidationError'}] }

  @Positive
  Scenario: POST a sharingSetting and get requests id as response
    # Create sharing setting for both centralTenant and universityTenant
    Given path 'consortia', consortiumId, 'sharing/settings'
    And header x-okapi-tenant = centralTenant
    And request
    """
    ...
    * match response.data.size() == 1
    * def createSettingsPCId = response.createSettingsPCId
    * match createSettingsPCId != null

    # Check details from publication request by using response createSettingsPCId
  ...
  Scenario: GET a sharingInstance by actionId
    ...
    * def createdInstance = response
    ...
    Given path 'consortia', consortiumId, 'sharing/settings', createdInstance.id
    And header x-okapi-tenant = centralTenant
    When method GET
    Then status 200
