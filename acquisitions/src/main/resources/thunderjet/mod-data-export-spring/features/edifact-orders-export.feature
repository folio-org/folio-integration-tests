Feature: EDIFACT orders export tests
  ...
  Scenario: If there is an open order for organization and organization has integration method with the same acquisition method as order THEN export job should be triggered and be 'SUCCESSFUL' (ediSchedulePeriod = 'DAY')
    ...
    * call read('this:util/initData.feature@CreateOrganization') { extOrganizationId: #(extOrganizationId), extOrganizationName: #(extOrganizationName), extOrganizationCode: #(extOrganizationCode) }

    # Create an account, and set it to the organization
    * def extAccount = read('this:samples/account-for-organization.json')
    ...
    * call read('this:util/initData.feature@SetAccountToOrganization') { extOrganizationId: #(extOrganizationId), extAccounts: [#(extAccount)] }

    # Create purchase order for the organization
    * def extOrderId =  call uuid1
    * def extPoNumber =  'PoNumber1'
    * call read('this:util/initData.feature@CreateOrderForOrganization') { extOrderId: #(extOrderId), extOrganizationId: #(extOrganizationId), extPoNumber: #(extPoNumber) }

    # Create PO Line for the order
    * def extPoLineId = call uuid1
    * call read('this:util/initData.feature@CreateOrderLines') { extPoLineId: #(extPoLineId), extOrderId: #(extOrderId), extOrganizationId: #(extOrganizationId), extAccountNumber: #(extAccountNo) }

    # Open the order
    * call read('this:util/initData.feature@OpenOrder') { extOrderId: #(extOrderId)}
    ...
    * call read('this:util/initData.feature@AddIntegrationToOrganization') { extExportConfigId: #(extExportConfigId), extOrganizationId: #(extOrganizationId), extAccountNoList: [#(extAccountNo)], extEdiScheduleFrequency: #(extEdiScheduleFrequency), extEdiSchedulePeriod: #(extEdiSchedulePeriod), extEdiScheduleTime: #(extEdiScheduleTime), extEdiScheduleTimeZone: #(extEdiScheduleTimeZone)}

    # Pause for a minute ('delay' = 1 minute)
    * call pause 65000

    # Verify that job run was successful
    * def dataExportSpringJobsByType = call read('this:util/initData.feature@GetDataExportSpringJobsByType')
  ...
  Scenario: If there is an open order for organization and organization has integration method with the same acquisition method as order THEN export job should be triggered and be 'SUCCESSFUL' (ediSchedulePeriod = 'WEEK')
    ...
    * def extEdiScheduleTimeZone = 'UTC'
    * call read('this:util/initData.feature@AddIntegrationToOrganization') { extExportConfigId: #(extExportConfigId), extOrganizationId: #(extOrganizationId), extAccountNoList: [#(extAccountNo)], extEdiScheduleFrequency: #(extEdiScheduleFrequency), extEdiSchedulePeriod: #(extEdiSchedulePeriod), extEdiScheduleTime: #(extEdiScheduleTime), extEdiScheduleWeekDays: #(extEdiScheduleWeekDays), extEdiScheduleTimeZone: #(extEdiScheduleTimeZone)}
