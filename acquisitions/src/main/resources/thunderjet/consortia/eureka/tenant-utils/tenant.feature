Feature: Tenant object in mod-consortia api tests
  ...
  Background:
    ...
    * configure headers = { 'Content-Type': 'application/json', 'x-okapi-token': '#(okapitoken)', 'x-okapi-tenant': '#(centralTenant.name)', 'Accept': 'application/json' }
  ...
  Scenario: Do POST a tenant, GET list of tenant(s) (isCentral = true), check value of 'setupStatus'
    ...
    Given path 'consortia', consortia.id, 'tenants'
    When method GET
    Then status 200
    ...
    Given path 'consortia', consortia.id, 'tenants'
    And request { id: '#(centralTenant.id)', code: 'ABC', name: 'Central tenants name', isCentral: true }
    When method POST
    Then status 201
    And match response == { id: '#(centralTenant.id)', code: 'ABC', name: 'Central tenants name', isCentral: true, isDeleted: false }

    # get tenant details for 'centralTenant'
    Given path 'consortia', consortia.id, 'tenants', centralTenant.name
    When method GET
    Then status 200
    And match response.id == centralTenant.id
    ...
    When method GET
    Then status 200
    And match response == { tenants: [{ id: '#(centralTenant.id)', code: 'ABC', name: 'Central tenants name', isCentral: true, isDeleted:false }], totalRecords: 1 }
    ...
    When method GET
    Then status 200
    And match response.centralTenantId == centralTenant.id
  ...
  Scenario: Do POST a non-central tenant (isCentral = false), GET list of tenant(s), check value of 'setupStatus'
    ...
    And request { id: '#(universityTenant.id)', code: 'XYZ', name: 'University tenants name', isCentral: false }
    When method POST
    Then status 201
    And match response == { id: '#(universityTenant.id)', code: 'XYZ', name: 'University tenants name', isCentral: false, isDeleted: false }

    # get tenant details for 'universityTenant'
    Given path 'consortia', consortia.id, 'tenants', universityTenant.name
    When method GET
    Then status 200
    And match response.id == universityTenant.id
    Then assert response.setupStatus == 'IN_PROGRESS' || response.setupStatus == 'COMPLETED'
    ...
    * call login universityUser1
    ...
    And match response.userTenants[0].tenantId == universityTenant.id