Feature: Find holdings by location and instance (central tenant only)

  Background:
    * url baseUrl
    * call login consortiaAdmin
    * configure headers = { 'Content-Type': 'application/xml', 'Authtoken-Refresh-Cache': 'true', 'x-okapi-token': '#(okapitoken)', 'x-okapi-tenant': '#(centralTenant)', 'Accept': '*/*' }
  ...
  Scenario: Create second order
    ...
    * def poLine = $.poLines[0]
    ...
    And param query = 'instanceId==' + poLine.instanceId + ' and permanentLocationId==' + centralLocationsId
    When method GET
    Then status 200
