...
Feature: Open order with member tenant location and verify instance, holding, and item creation [MODORDSTOR-402]
  ...
  Scenario: Prepare data: create fund and budget
    ...
    * def v = call createBudget { id: '#(budgetId)', fundId: '#(fundId)', allocated: 100, status: 'Active' }
  ...
  @Positive
  Scenario: Change instance connection 'MOVE' for the order to some member tenant, where the shadow instance is located. Verify that associated holdings and items moved to the same tenant
    ...
    * def requestEntity = { operation: 'Replace Instance Ref', replaceInstanceRef: { holdingsOperation: 'Move', newInstanceId: '#(centralPoLinInstanceId2)' }}
    ...
    # 8. Verify Instance, Holdings and Items 'centralTenant'. Check both Holdings moved to 'centralPoLineInstanceId2'
    Given path 'inventory/instances', centralPoLinInstanceId2
    When method GET
    Then status 200
    ...
    # 9. Verify Shadow Instance, Holdings and items moving from 'centralPoLinInstanceId1' to 'centralPoLinInstanceId2' after changing instance connection in 'universityTenant'
    * configure headers = headersUni
  ...
  @Positive
  Scenario: Change instance connection 'FIND_OR_CREATE' for the order to some member tenant, where the shadow instance is located. Verify that associated holdings and items moved to the same tenant
    # 1. Create Order and orderLine
    # 1.1 Create orders
    * def v = call createOrder { id: '#(orderId)', vendor: '#(centralVendorId)', orderType: 'One-Time', ongoing: null }
    ...
    * def universityPoLineItemId2 = response.items[1].id
    ...
    # 5. Change 'first' poLine instance 'centralInstanceId1' connection to the 'instanceId1' that has shadow instance in 'universityTenant
    * configure headers = headersCentral
    ...
    * def newCentralPoLineHoldingId2 = response.holdingsRecords[1].id
    ...
    # 7. Verify Shadow Instance, Holdings and items moving from 'centralPoLinInstanceId1' to 'instanceId1' after changing instance connection in 'universityTenant'
    * configure headers = headersUni
    Given path 'inventory/instances', instanceId1
    When method GET
    Then status 200
  ...
  @Positive
  Scenario: Change instance connection 'CREATE' for the order to some member tenant, where the shadow instance is located. Verify that associated holdings and items moved to the same tenant
    # 1. Create Order and orderLine
    # 1.1 Create orders
    * def v = call createOrder { id: '#(orderId)', vendor: '#(centralVendorId)', orderType: 'One-Time', ongoing: null }
    ...
    * def universityPoLineItemId2 = response.items[1].id
    ...
    # 5. Change 'first' poLine instance 'centralInstanceId1' connection to the 'instanceId2' that has shadow instance in 'universityTenant
    * configure headers = headersCentral
