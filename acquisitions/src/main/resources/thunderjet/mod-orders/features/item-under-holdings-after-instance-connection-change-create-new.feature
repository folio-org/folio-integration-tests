# For MODORDERS-730, https://foliotest.testrail.io/index.php?/cases/view/358536
Feature: Item Appears Under Holdings After Instance Connection Change With Holding Setting Create New
  ...
  Scenario: Item Appears Under Holdings After Instance Connection Change With Holding Setting Create New
    ...
    * table testLocations
      ...
      | locationId1   | 'LOC1-CN-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
      | locationId2   | 'LOC2-CN-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
      | locationId3   | 'LOC3-CN-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
    ...
    * def v = call createInstance { id: '#(newInstanceId)', title: 'Title #2 Create New', instanceTypeId: '#(globalInstanceTypeId)' }
    ...
    * def orderLineRequest =
    """
    {
      id: '#(poLineId)',
      orderId: '#(orderId)',
      titleOrPackage: 'Title #1 Create New',
    ...
    * def v = call receivePieceWithHolding { pieceId: '#(pieceId3)', poLineId: '#(poLineId)', holdingId: '#(holdingId3)' }
    ...
    # 6. Change Instance Connection To Title #2 With "Create" Holdings Operation And Keep Abandoned Holdings
    * print '6. Change Instance Connection To Title #2 With "Create" Holdings Operation And Keep Abandoned Holdings'
    * def v = call changeOrderLineInstanceConnection { poLineId: '#(poLineId)', instanceId: '#(newInstanceId)', holdingsOperation: 'Create', deleteAbandonedHoldings: false }

    # 7. Verify PO Line Instance ID Updated And Holdings Replaced With New Ones
    * print '7. Verify PO Line Instance ID Updated And Holdings Replaced With New Ones'
    * def isPoLineUpdated =
    """
    function(response) {
    ...
    # 8.1 Verify Title #2 Has Three Newly Created Holdings (Loc1, Loc2, Loc3)
    * print '8.1 Verify Title #2 Has Three Newly Created Holdings (Loc1, Loc2, Loc3)'
    * configure headers = headersAdmin
