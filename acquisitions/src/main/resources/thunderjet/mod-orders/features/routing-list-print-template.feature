...
Feature: Should check processing of printing routing list functionlity
  ...
  3. Verify details of template response
    ...
    * def expectedBody = '<div><p>Biography room</p> <p>title 1</p> <p>Falcon Denesik - 113 Law</p><p>Artimus Denesik - 143 Law</p><p>John Huels - 188 Law</p><p>Some note</p></div>'
    * def isTemplateReady =
      """
      function(response) {
        return response.map != null &&
               response.map.result != null &&
               response.map.result.body == expectedBody
      }
      """
    Given path 'orders/routing-lists/' + routingListId + '/template'
    And retry until responseStatus == 200 && isTemplateReady(response)
    When method GET
    Then status 200
