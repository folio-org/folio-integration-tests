...
Feature: Test operations affecting pieces with different po line options
  ...
  Background:
    ...
    * def noLocation = []
    * def normalLocation = [ { "locationId": "#(locationId)", "quantity": 2, "quantityElectronic": 1, "quantityPhysical": 1 } ]
    * def holdingLocation = [ { "holdingId": "#(initialHoldingId)", "quantity": 2, "quantityElectronic": 1, "quantityPhysical": 1 } ]
