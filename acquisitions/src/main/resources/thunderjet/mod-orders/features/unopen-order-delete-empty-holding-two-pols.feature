# For MODORDERS-1410, https://foliotest.testrail.io/index.php?/cases/view/1273166
Feature: Unopen Order With 2 Independent POLs Deletes Only Empty Holding
  ...
  @C1273166
  @Positive
  Scenario: Unopen Order With 2 Independent POLs And Piece On POL1 - Only POL2 Empty Holding Deleted
    ...
    * def v = call createBudget { id: '#(budgetId)', allocated: 10000, fundId: '#(fundId)', status: 'Active' }
    ...
    # 2. Create Order With 2 POLs - Both Independent Workflow, Qty 1, Instance Holdings
    * print '2. Create Order With 2 POLs - Both Independent Workflow, Qty 1, Instance Holdings'
    * configure headers = headersUser
    * def v = call createOrder { id: '#(orderId)', orderType: 'One-Time' }
    * def v = call createOrderLine { id: '#(poLine1Id)', orderId: '#(orderId)', fundId: '#(fundId)', quantity: 1, createInventory: 'Instance, Holding', checkinItems: true }
    * def locations2 =
    """
    [
    ...
    * def v = call openOrder { orderId: '#(orderId)' }
    ...
    # 4. Get Instance IDs And Retrieve Holdings Created
    * print '4. Get Instance IDs And Retrieve Holdings Created'
    ...
    * configure headers = headersUser
    ...
    # 5. Get Title For POL1 And Add Piece To POL1 Holding
    * print '5. Get Title For POL1 And Add Piece To POL1 Holding'
    Given path 'orders/titles'
    And param query = 'poLineId==' + poLine1Id
    When method GET
    Then status 200
    And match $.totalRecords == 1
    * def title1Id = $.titles[0].id

    * def v = call createPieceWithHoldingOrLocation { id: '#(pieceId)', poLineId: '#(poLine1Id)', titleId: '#(title1Id)', holdingId: '#(holding1Id)' }
    ...
    * print '7. Verify Order Workflow Status Is "Pending"'
    ...
    # 8. Verify POL1 Holding Still Exists (Has A Piece)
    * print '8. Verify POL1 Holding Still Exists (Has A Piece)'
    * configure headers = headersAdmin
    Given path 'holdings-storage/holdings', holding1Id
    When method GET
    Then status 200
    And match $.permanentLocationId == locId

    # 9. Verify POL2 Holding Is Deleted (Was Empty - No Piece)
    * print '9. Verify POL2 Holding Is Deleted (Was Empty - No Piece)'
