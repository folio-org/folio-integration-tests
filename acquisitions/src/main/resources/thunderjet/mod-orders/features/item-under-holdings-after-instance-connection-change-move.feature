# For MODORDERS-730, MODORDSTOR-483, https://foliotest.testrail.io/index.php?/cases/view/358533
Feature: Item Appears Under Holdings After Instance Connection Change With Holding Setting Move
  ...
  Scenario: Item Appears Under Holdings After Instance Connection Change With Holding Setting Move
    ...
    * table testLocations
      ...
      | locationId1   | 'LOC1-MV-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
      | locationId2   | 'LOC2-MV-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
      | locationId3   | 'LOC3-MV-' + codeSuffix    | globalLocationUnitsInstructionsId   | globalLocationUnitsCampusesId  | globalLocationUnitsLibrariesId  | globalServicePointsId |
    ...
    * def v = call createInstance { id: '#(newInstanceId)', title: 'Title #2 Move', instanceTypeId: '#(globalInstanceTypeId)' }
    ...
    * def v = call receivePieceWithHolding { pieceId: '#(pieceId3)', poLineId: '#(poLineId)', holdingId: '#(holdingId3)' }
    ...
    # 6. Change Instance Connection To Title #2 With "Move" Holdings Operation
    * print '6. Change Instance Connection To Title #2 With "Move" Holdings Operation'
    * def v = call changeOrderLineInstanceConnection { poLineId: '#(poLineId)', instanceId: '#(newInstanceId)', holdingsOperation: 'Move', deleteAbandonedHoldings: false }

    # 7. Verify PO Line Instance ID Updated And Existing Holdings Are Reused
    * print '7. Verify PO Line Instance ID Updated And Existing Holdings Are Reused'
    * def isPoLineUpdated =
    """
    function(response) {
      ...
      for (var i = 0; i < response.locations.length; i++) {
        ...
        if (id == holdingId1) found1 = true;
        if (id == holdingId2) found2 = true;
      }
    ...
    # 8.1 Verify Title #2 Now Owns The Three Original Holdings (Loc1, Loc2, Loc3)
    * print '8.1 Verify Title #2 Now Owns The Three Original Holdings (Loc1, Loc2, Loc3)'
    * configure headers = headersAdmin
    ...
    # 8.2 Verify Two Received Items Exist For The PO Line And Are Linked To Moved Holdings
    * print '8.2 Verify Two Received Items Exist For The PO Line And Are Linked To Moved Holdings'
    ...
    # 9. Verify Title #1 No Longer Has Any Holdings Or Items
    * print '9. Verify Title #1 No Longer Has Any Holdings Or Items'
    Given path 'holdings-storage/holdings'
