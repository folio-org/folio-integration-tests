...
Feature: Helper for "check-invoice-lines-with-vat-adjustments.feature"
  ...
  Scenario: Check invoices with applied top adjustment
    ...
    And match each response.adjustments[*].type == 'Percentage'
  ...
  @CheckInvoiceLinesWithAppliedTopAdjustment #(id, invoiceLines, lineTotal, lineSubTotals, lineAdjustmentsTotal, lineAdjustments, status, adjustmentId)
  Scenario: Check invoice line with applied top adjustment
    ...
    And match each response.invoiceLines[*].adjustmentsTotal == lineAdjustmentsTotal
    ...
    And match response.invoiceLines[*].adjustments[*].value contains lineAdjustments
  ...
  @CheckInvoiceLinesWithAppliedIndividualAdjustment #(id, invoiceLines, lineTotal, lineSubTotals, lineAdjustmentsTotal, lineAdjustments, status)
  Scenario: Check invoice line with applied individual adjustment
    ...
    And match each response.invoiceLines[*].invoiceLineStatus == status
    And match response.invoiceLines[*].adjustments[*].value contains lineAdjustments
    And match each response.invoiceLines[*].adjustments[*].type == 'Percentage'
    And match each response.invoiceLines[*].adjustments[*].prorate == 'Not prorated'
    And match each response.invoiceLines[*].adjustments[*].description == 'VAT'
    And match each response.invoiceLines[*].adjustments[*].relationToTotal == 'Included in'