...
class ListGetSimulation extends Simulation {
  ...
  val perform = scenario("perform")
    .repeat(10) {
      exec(karateFeature("classpath:corsair/mod-lists/features/get-lists.feature"))
    }
    .repeat(10) {
      exec(karateFeature("classpath:corsair/mod-lists/features/get-list-contents.feature"))
    }
