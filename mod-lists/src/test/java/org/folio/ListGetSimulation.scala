...
class ListGetSimulation extends Simulation {
  ...
  val perform = scenario("perform")
    .repeat(10) {
      exec(karateFeature("classpath:athena/mod-lists/features/get-lists.feature"))
    }
    .repeat(10) {
      exec(karateFeature("classpath:athena/mod-lists/features/get-list-contents.feature"))
    }
