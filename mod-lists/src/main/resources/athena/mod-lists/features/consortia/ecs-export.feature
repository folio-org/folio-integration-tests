Feature: Scenarios that are primarily focused around exporting list data for ECS TODO: UPDATE
  ...
  Scenario: Test export list
    ...
    * def listRequest = read('classpath:athena/mod-lists/features/samples/' + listFile)
    ...
    * def refreshStatus = response.successRefresh != null ? 'SUCCESS' : 'FAILED'
    ...
    # Retry if failed
    * if (refreshStatus == 'FAILED') karate.call('classpath:athena/mod-lists/features/util/refresh-list.feature', { listId: listId })
    ...
    * def columnsResponse = providedFields == null ? karate.call('classpath:athena/mod-lists/features/util/get-all-columns.feature', { entityTypeId: entityTypeId }) : null
