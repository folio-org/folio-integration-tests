Feature: data-import-large-scale-tests create-electronic-only-match integration tests
  ...
  Background:
    ...
    * def recordFilesDir = 'classpath:promin/data-import-large-scale-tests/samples/records/marc/'
    * def importFile = read('classpath:promin/data-import-large-scale-tests/global/import-file.feature')
    * def getJobLogEntriesByJobId = read('classpath:promin/data-import-large-scale-tests/global/data-import-logs.feature@getJobLogEntriesByJobId')

  Scenario: FAT-20172 Create Electronic only match

    * print 'FAT-20172-create-electronic-only-match.feature'
    * def updateProfileSnapshot = read('classpath:promin/data-import-large-scale-tests/samples/profiles/FAT-20172-create-electronic-only-match.json')
    ...
    * def verifyLogsInstanceCreated = function(job) { karate.call('classpath:promin/data-import-large-scale-tests/features/marc-bib/create/create-instance.feature@verifyInstanceCreated', { headersUser, jobId: job.id, limit: logEntriesLimit }) }
    ...
    * def processJobExecution =
        ...
        function(jobs, pass, headersUser) {
          ...
          for (var i = 0; i < jobs.length; i++) {
            ...
            var res;
            if (pass) {
              res = karate.call('classpath:promin/data-import-large-scale-tests/global/FAT-20172-verify-success.feature', { headersUser: headersUser, jobId: jobId, limit: limit });
            } else {
              res = karate.call('classpath:promin/data-import-large-scale-tests/global/FAT-20172-verify-failure.feature', { headersUser: headersUser, jobId: jobId, limit: limit });
