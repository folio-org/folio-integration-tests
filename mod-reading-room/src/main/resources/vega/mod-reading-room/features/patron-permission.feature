Feature: PatronPermission tests
  ...
  Scenario: update patron permission
    ...
    * def servicePointEntityRequest = read('classpath:vega/mod-reading-room/features/samples/service-point/service-point-entity-request.json')
    Given path 'service-points'
    And request servicePointEntityRequest
    When method POST
    Then status 201

    * def readingRoomEntityRequest = read('classpath:vega/mod-reading-room/features/samples/reading-room/reading-room-entity-request.json')
    Given path 'reading-room'
    And request readingRoomEntityRequest
    When method POST
    Then status 201

    * def patronPermissionEntityRequest = read('classpath:vega/mod-reading-room/features/samples/patron-permission/patron-permission-entity-request.json')
