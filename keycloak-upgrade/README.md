...
When no state file exists, the seed phase generates random central/member tenant names and ids, then writes them to `target/keycloak-upgrade-tenant.properties`. The verify phase reads the same file.
...
```shell
mvn clean test -pl common,common-consortia,testrail-integration,keycloak-upgrade \
  -DfailIfNoTests=false \
  -Dsurefire.failIfNoSpecifiedTests=false \
  -Dtest=KeycloakUpgradeSeedTests \
  -DargLine="-Dkarate.env=eureka1"
```
...
```shell
mvn test -pl common,common-consortia,testrail-integration,keycloak-upgrade \
  -DfailIfNoTests=false \
  -Dsurefire.failIfNoSpecifiedTests=false \
...
To use an explicit central tenant instead of the generated one, add both `-DtestTenant=<tenant>` and `-DtestTenantId=<uuid>` to the seed command. The seed phase still writes all fixture ids to the state file for the verify command.
...
- Consortia non-primary affiliation deletion/re-addition still drives shadow user deactivation/reactivation through Keycloak-aware user updates.
- The verify phase cleans up the generated central and member tenants after a successful run.
