...
Feature: Test enhancements to oai-pmh
  ...
  Scenario Outline: set errors to 200 and 500 and check Http status in responses <errorCode>
    ...
    * call read('classpath:athena/mod-configuration/reusable/mod-config-templates.feature')
    * copy valueTemplate = behaviorValue
    * def valueTemplateString = valueTemplate
    * print 'valueTemplate=', valueTemplate
    * call read('classpath:athena/mod-configuration/reusable/update-configuration.feature@BehaviorConfig') {id: '#(behaviorId)', data: '#(valueTemplateString)'}
  ...
  Scenario Outline: check enable and disable OAI service <enableOAIService>
    ...
    * def valueTemplateString = valueTemplate
    * print 'valueTemplate=', valueTemplate
    * call read('classpath:athena/mod-configuration/reusable/update-configuration.feature@GeneralConfig') {id: '#(generalId)', data: '#(valueTemplateString)'}
  ...
  Scenario: check noRecordsMatch in ListRecords request for marc21_withholdings
    * call read('classpath:athena/mod-configuration/reusable/update-configuration.feature@SetErrorProcessing500')
    And param verb = 'ListRecords'
    And param metadataPrefix = 'marc21_withholdings'
  ...
  Scenario Outline: get resumptionToken for ListRecords and make responses until resumptionToken is present for <prefix>
    ...
    * def valueTemplateString = valueTemplate
    * call read('classpath:athena/mod-configuration/reusable/update-configuration.feature@TechnicalConfig') {id: '#(technicalId)', data: '#(valueTemplateString)'}
    * def totalRecords = 0
  ...
  Scenario: set config "deletedRecordsSupport" by default to "persistent" and find record marked as deleted by header with status = deleted in response
    ...
    * def record = response
    * def instanceId = record.externalIdsHolder.instanceId

    # set inventory instance deleted = true
    Given url baseUrl
    ...
    * set instance.deleted = true
    ...
    Then status 200
    * match response count(//record) == 9
    * match response count(//header[@status='deleted']) == 0
  ...
  Scenario: Verify that resumption Token contains tenantId and all the other required parameters
    ...
    * call read('classpath:athena/mod-configuration/reusable/update-configuration.feature@TechnicalConfig') {id: '#(technicalId)', data: '#(valueTemplateStringTechnical)'}
