Feature: Users tests
  ...
  Scenario: Create a new User with PatronGroup.
    ...
    * def uuid = call uuid1
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') {status: true, uuid: #(uuid), username: #(username), barcode: #(barcode)}

  Scenario: Search user by barcode.
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { barcode: #(barcode), status: true, uuid: #(uuid), username: #(username)}
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { barcode: #(barcode), status: true, uuid: #(uuid), username: #(username)}
...
#    And param query = '(userId==' + extUserId + ' and ' + 'itemId==' + extItemId + ')'
  ...
  Scenario:  Find an active user and make that user the sponsor of another active patron
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * def createUserResponse = call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser')
    * def userId = createUserResponse.response.id
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * def createProxyUserResponse = call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser')
  ...
  Scenario: Search user by firstname.
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { firstName: abc, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * def createUserResponse = call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { firstName: xyz, status: true, uuid: #(uuid), username: #(username) }
  ...
  Scenario: Search user by firstname & lastname.
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { firstName: abc,lastName: xyz, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { firstName: pqr,lastName: def, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
  ...
  Scenario: Search user by UUID.
    ...
    * def uuid = call uuid1
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { uuid: #(uuid), status: true, username: #(username), barcode: #(barcode) }
    * def barcode = call random_numbers
    * def username = call random_string
    * def uuid = call uuid1
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { uuid: #(uuid), status: true, username: #(username), barcode: #(barcode) }
  ...
  Scenario: Search user by lastname.
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { lastName: abc, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { lastName: pqr, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
  ...
  Scenario: Search user by email.
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { email: testmail@abc.com, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
    * def uuid = call uuid1
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { email: abc@xyz.com, status: true, uuid: #(uuid), username: #(username), barcode: #(barcode) }
  ...
  Scenario: Filter inactive patron.
    ...
    * def username = call random_string
    * def barcode = call random_numbers
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUser') { status: false, uuid: #(uuid), username: #(username), barcode: #(barcode) }
  ...
  Scenario: Update User's Profile Picture
    ...
    * def username = call random_string
    * def uuid = call uuid1
    * call read('classpath:volaris/mod-users/eureka-features/util/initData.feature@PostPatronGroupAndUserWithProfilePicture') { status: true, uuid: #(uuid), username: #(username), barcode: #(barcode)}
