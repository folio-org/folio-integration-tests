Feature: setup tenant
  ...
  Scenario: Post tenant, enable all required modules, and setup admin
    ...
    * call read('consortia/util/initData.feature@InstallModules') { modules: [{name: 'mod-authtoken'}], tenant: '#(tenant)'}
    ...
    * def disabledResponse = call read('consortia/util/initData.feature@DisableModules') { modules: [{name: 'mod-authtoken'}], tenant: '#(tenant)'}
    ...
    * call read('consortia/util/initData.feature@SetUpAdmin') admin
    ...
    # # install mod-authtoken module
    * call read('consortia/util/initData.feature@Install') { disabledResponse: '#(disabledResponse)', tenant: '#(tenant)'}
