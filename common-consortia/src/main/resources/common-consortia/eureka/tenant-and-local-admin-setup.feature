...
#     install required modules
    ...
    * call read('classpath:common-consortia/eureka/initData.feature@InstallModules') { tenant: '#(tenant)', modules: '#(modules)', token: '#(token)'}
#
#     set up 'admin-user' with all existing permissions of enabled modules
    * print 'SetUpAdmin (#(tenant))'
    * call read('classpath:common-consortia/eureka/initData.feature@SetUpAdmin') {tenant: '#(tenant)', user: '#(adminUser)', token: '#(token)'}

#     enable 'folio_users' (requires 'mod-tags', 'mod-users-bl', 'mod-password-validator')
    * print 'InstallModules (#(tenant))'
    * call read('classpath:common-consortia/eureka/initData.feature@InstallModules') { tenant: '#(tenant)', modules: '#(requiredModulesForConsortia)', token: '#(token)'}

#    # enable 'mod-consortia-keycloak' app-consortia-1.0.0-SNAPSHOT.1358
    * call read('classpath:common-consortia/eureka/initData.feature@InstallApplications') { applicationIds: ['app-consortia-1.0.0-SNAPSHOT.1358'], tenant: '#(tenant)', token: '#(token)'}
