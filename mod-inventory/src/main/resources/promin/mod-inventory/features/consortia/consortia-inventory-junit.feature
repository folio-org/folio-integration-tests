Feature: mod-inventory ECS tests
  ...
  Background:
    ...
    * def samplesPath = 'classpath:promin/mod-inventory/samples/'
  ...
  Scenario: Update hrId for all tenants
    * call login consortiaAdmin
    * call read('classpath:promin/mod-inventory/features/consortia/util/hrid-util.feature@UpdateHrId') { tenant: '#(centralTenant)', prefix: 'cons' }

    * call login universityUser1
    * call read('classpath:promin/mod-inventory/features/consortia/util/hrid-util.feature@UpdateHrId') { tenant: '#(universityTenant)', prefix: 'u' }

    * call login collegeUser1
    * call read('classpath:promin/mod-inventory/features/consortia/util/hrid-util.feature@UpdateHrId') { tenant: '#(collegeTenant)', prefix: 'o' }

  Scenario: Create locations
    Given call read('classpath:promin/mod-inventory/features/locations.feature') { testUser : '#(consortiaAdmin)' }
    Given call read('classpath:promin/mod-inventory/features/locations.feature') { testUser : '#(universityUser1)' }
    Given call read('classpath:promin/mod-inventory/features/locations.feature') { testUser : '#(collegeUser1)' }
  ...
  Scenario: create holdings source type
    * def holdingsSource = read('classpath:promin/mod-inventory/samples/holdings_source.json')
