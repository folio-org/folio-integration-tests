Feature: Setup quickMARC
  ...
  Background:
    ...
    * def samplePath = 'classpath:promin/mod-quick-marc/features/setup/samples/'
  ...
  Scenario: Create quickMARC Bib record
    ...
    * recordPayload.fields.push(field001)
    ...
    And def recordId = response.externalId
     ...
     #Check srs creation
    * def srsRecord = call read('classpath:promin/mod-quick-marc/features/setup/setup.feature@GetSRSRecord') {recordId: '#(recordId)', idType: 'INSTANCE'}
    * match srsRecord.response.deleted == false
    * match srsRecord.response.state == 'ACTUAL'

    * call read('classpath:promin/mod-quick-marc/features/setup/setup.feature@GetRecordById') {recordId: '#(recordId)'}
  ...
  Scenario: Create quickMARC Holding record
    ...
    * record.fields.push(field001)
    ...
    And def recordId = response.externalId
      ...
      #Check srs creation
    * def srsRecord = call read('classpath:promin/mod-quick-marc/features/setup/setup.feature@GetSRSRecord') {recordId: '#(recordId)', idType: 'HOLDINGS'}
    * match srsRecord.response.deleted == false
    * match srsRecord.response.state == 'ACTUAL'
  ...
  Scenario: Create quickMARC Authority record
    ...
    * set record._actionType = 'create'
    ...
    And def recordId = response.externalId
      ...
      #Check srs creation
    * def srsRecord = call read('classpath:promin/mod-quick-marc/features/setup/setup.feature@GetSRSRecord') {recordId: '#(recordId)', idType: 'AUTHORITY'}
    * match srsRecord.response.deleted == false
    * match srsRecord.response.state == 'ACTUAL'
  ...
  Scenario: Create bib record with 100 and 600 authority links
    ...
    * call read('classpath:promin/mod-quick-marc/features/setup/setup.feature@CreateMarcBib') {id: '#(newBibId)', hrid: '#(newBibHrid)'}
