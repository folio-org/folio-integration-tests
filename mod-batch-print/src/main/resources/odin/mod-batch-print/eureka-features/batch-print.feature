Feature: Batch print
  ...
  Scenario: Test CRUD operations
    * def requestEntity = read('classpath:odin/mod-batch-print/features/samples/print-entry.json')
    * def requestEntityUpdated = read('classpath:odin/mod-batch-print/features/samples/print-entry-updated.json')
    * def entryId = '47c62bf9-e225-4a4b-bb61-dc6fc11eed76'
    * configure headers = { 'Content-Type': 'application/json', 'x-okapi-url': '#(baseUrl)', 'x-okapi-tenant': '#(testTenant)', 'x-okapi-token': '#(okapitoken)', 'Accept': 'application/json, text/plain' }
...
#      Given path '/print/entries/' + entryId
  ...
  Scenario: Create print entry by notice template
    * def requestEntity = read('classpath:odin/mod-batch-print/features/samples/mail.json')
