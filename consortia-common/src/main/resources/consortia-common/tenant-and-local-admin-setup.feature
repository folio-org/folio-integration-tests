Feature: setup tenant
  ...
  Scenario: Post tenant, enable all required modules, and setup admin
    ...
    * call read('classpath:consortia-common/initData.feature@PostTenant') { id: '#(tenant)', name: '#(name)', description: '#(description)'}

    # install required modules
    * print 'InstallModules (#(tenant))'
    * call read('classpath:consortia-common/initData.feature@InstallModules') { modules: '#(requiredModules)', tenant: '#(tenant)'}

    # set up 'admin-user' with all existing permissions of enabled modules
    * print 'SetUpAdmin (#(tenant))'
    * call read('classpath:consortia-common/initData.feature@SetUpAdmin') admin
