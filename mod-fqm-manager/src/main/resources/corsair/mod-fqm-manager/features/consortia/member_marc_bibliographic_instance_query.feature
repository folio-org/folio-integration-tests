Feature: Query member tenant instances with MARC bibliographic in ECS
  ...
  Scenario: [Instances with MARC bibliographic] [Member tenant] Query returns local and shared instances
    ...
    * def sharedInstanceHrid = 'fqm-mbi-shared-' + sharedInstanceId
    * def memberInstanceHrid = 'fqm-mbi-member-' + memberInstanceId
    * def sharedInstanceTitle = 'ECS MARC bibliographic instance query ' + randomMillis()
    * def sharingId = uuid()
    * def instanceTypeRequest = { id: '#(instanceTypeId)', name: 'ECS MARC bibliographic integration test type', code: 'ecs-mbi', source: 'local' }
    ...
    * def queryId = response.queryId
    ...
    When method GET
    Then status 200
    * if (response.status == 'FAILED') karate.fail('FQM member MARC bibliographic instance query failed: ' + karate.pretty(response))
