Feature: Add FQM query data
  ...
  Scenario: Add sample data needed for FQM queries
    ...
    * def departmentRequest = {code: 'DEPT', name: 'Test department', id: '310f6067-4fd0-5108-a589-cb429c5c7973'}
    ...
    # Add users
    * def userRequest = read('classpath:athena/mod-fqm-manager/features/samples/user-request.json')
    Given path 'users'
    ...
    * def userRequest = read('classpath:athena/mod-fqm-manager/features/samples/user-request-with-address.json')
    Given path 'users'
    And request userRequest
    When method POST
    Then status 201

    * def userRequest = read('classpath:athena/mod-fqm-manager/features/samples/user-request-missing-address-fields.json')
    Given path '/users'
    And request userRequest
    When method POST
    Then status 201
    ...
    * def marcParsedContent = read('classpath:athena/mod-fqm-manager/features/samples/marc-bib-parsed-content.json')
