Feature: Pickup Flow Update
  ...
  Scenario: Create DCB PICKUP transaction, update item fields and verify requests and transaction statuses
    ...
    * def createInst = call read('classpath:vega/mod-dcb/reusable/create-institution.feature') { name: 'Shadow Institution C1307939', code: '#(shadowInstCode)', isShadow: true }
    And match createInst.response.isShadow == true
    * def shadowInstitutionId = createInst.response.id
    * def createCamp = call read('classpath:vega/mod-dcb/reusable/create-campus.feature') { name: 'Shadow Campus C1307939', code: '#(shadowCampCode)', institutionId: '#(shadowInstitutionId)', isShadow: true }
    And match createCamp.response.isShadow == true
    * def shadowCampusId = createCamp.response.id
    * def createLib = call read('classpath:vega/mod-dcb/reusable/create-library.feature') { name: 'Shadow Library C1307939', code: '#(shadowLibCode)', campusId: '#(shadowCampusId)', isShadow: true }
    And match createLib.response.isShadow == true
    * def shadowLibraryId = createLib.response.id
    * def createLoc = call read('classpath:vega/mod-dcb/reusable/create-location.feature') { name: 'Shadow Location C1307939', code: '#(shadowLocCode)', institutionId: '#(shadowInstitutionId)', campusId: '#(shadowCampusId)', libraryId: '#(shadowLibraryId)', servicePointId: '#(servicePointId)', isShadow: true }
