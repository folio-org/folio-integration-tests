Feature: Testing Lending Flow
  ...
  Background:
    ...
    * callonce read('classpath:vega/mod-dcb/global/variables.feature')
  ...
  Scenario: current item check-in with non-existing barcode item
    * def intCheckInDate = call read('classpath:vega/mod-dcb/features/util/get-time-now-function.js')
    * def checkInRequest2 = read('classpath:vega/mod-dcb/features/samples/check-in/check-in-by-non-existing-barcode-entity-request.json')
