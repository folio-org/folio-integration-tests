Feature: Borrowing Flow Scenarios
  ...
  Scenario: Validation. If the userId and barcode is not exist already, error will be thrown.
    ...
    * url baseUrlNew
    * def createDCBTransactionRequest = read('classpath:vega/mod-dcb/features/samples/transaction/create-dcb-transaction.json')
  ...
  Scenario: Get Transaction status list after Cancelled
    ...
    * def newPath = proxyCall == true ? proxyPath+orgPath : orgPath
    * def pollConfig = { expectedRecords: 1, path: '#(newPath)', apikey: '#(key)', baseUrl: '#(baseUrlNew)', startDate: '#(startDate)' }

    Given def pollResult = call read("classpath:vega/mod-dcb/reusable/poll-transaction-statuses.feature@PollTransactionStatuses") { config: '#(pollConfig)' }
  ...
  Scenario: Get Transaction status list after Check out
    ...
    * def orgPath = '/transactions/status'
    * def newPath = proxyCall == true ? proxyPath+orgPath : orgPath
    * def pollConfig = { expectedRecords: 4, pageSize: 3, pageNumber: 1, path: '#(newPath)', apikey: '#(key)', baseUrl: '#(baseUrlNew)', startDate: '#(startDate)' }
  ...
  Scenario: Get Transaction status list after Check In
    ...
    * def orgPath = '/transactions/status'
    * def newPath = proxyCall == true ? proxyPath+orgPath : orgPath
    * def pollConfig = { expectedRecords: 5, pageSize: 3, pageNumber: 1, path: '#(newPath)', apikey: '#(key)', baseUrl: '#(baseUrlNew)', startDate: '#(startDate)' }
  ...
  Scenario: Get Transaction status list after Closed
    ...
    * def orgPath = '/transactions/status'
    * def newPath = proxyCall == true ? proxyPath+orgPath : orgPath
    * def pollConfig = { expectedRecords: 6, pageSize: 1, pageNumber: 5, path: '#(newPath)', apikey: '#(key)', baseUrl: '#(baseUrlNew)', startDate: '#(startDate)' }

    Given def pollResult = call read("classpath:vega/mod-dcb/reusable/poll-transaction-statuses.feature@PollTransactionStatuses") { config: '#(pollConfig)' }
    Then def response = pollResult.response
    And match response.totalRecords == 6
    And match response.maximumPageNumber == 5
    And match response.transactions[0].id == dcbTransactionId31
