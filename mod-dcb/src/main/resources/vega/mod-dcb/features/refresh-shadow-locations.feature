Feature: Borrowing Flow Scenarios
  ...
  Scenario: Create a shadow location from location with agency for borrower transaction
    ...
    * def args = { name: '#(dcbAgency.name)', code: '#(dcbAgency.code)', isShadow: true }
    Given call read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@GetInstitutionByNameAndCode') args
    Given call read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@GetCampusByNameAndCode') args
    Given call read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@GetLibraryByNameAndCode') args

    * def args = { name: '#(dcbLocation1.name)', code: '#(dcbLocation1.code)', isShadow: true }
    Given call read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@GetLocationByNameAndCode') args
