Feature: Testing Borrowing-Pickup Flow Request Expiration
  ...
  Scenario: Request expiration for BORROWING_PICKUP role transitions transaction to EXPIRED, check-in on any service point closes transaction
    ...
    * def createRequest = read('classpath:vega/mod-dcb/features/samples/transaction/create-dcb-transaction-for-borrowing-pickup.json')
    ...
    * def updateResult = call read('classpath:vega/mod-dcb/features/util/schedulerUtil.feature@UpdateRequestExpirationTimer') { extToken: #(sidecarToken), extUnit: 'second', extDelay: '1' }
    ...
    * if (updateResult.currentTimerCreated) karate.call('classpath:vega/mod-dcb/features/util/schedulerUtil.feature@DeleteRequestExpirationTimer', { extToken: sidecarToken, extTimerId: currentTimerId })
    * if (!updateResult.currentTimerCreated) karate.call('classpath:vega/mod-dcb/features/util/schedulerUtil.feature@UpdateRequestExpirationTimer', { extToken: sidecarToken, extTimerId: currentTimerId, extModuleId: updateResult.currentModuleId, extModuleName: updateResult.currentModuleName, extUnit: 'minute', extDelay: '30' })
    * configure headers = headersUser
    * configure retry = { count: 10, interval: 5000 }
    ...
    * def intCheckInDate2 = call read('classpath:vega/mod-dcb/features/util/get-time-now-function.js')
    * def checkInRequest2 = read('classpath:vega/mod-dcb/features/samples/check-in/check-in-by-barcode-entity-request.json')
