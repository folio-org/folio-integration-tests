Feature: Testing Lending Flow Request Expiration
  ...
  Scenario: Request expiration for LENDER role transitions transaction to EXPIRED, check-in at any service point closes transaction
    * def newItemPayload = read('classpath:vega/mod-dcb/features/samples/item/item-entity-request.json')
    ...
    * def createRequest = read('classpath:vega/mod-dcb/features/samples/transaction/create-dcb-transaction.json')
