Feature: Lender Renew Status Restriction
  ...
  Scenario: Create LENDER DCB transaction and verify renew is restricted to ITEM_CHECKED_OUT status only
    ...
    * def itemReq = read('classpath:vega/mod-dcb/features/samples/item/item-entity-request.json')
    ...
    * def updateToCheckedInRequest = read('classpath:vega/mod-dcb/features/samples/transaction/update-dcb-transaction-to-item-check-in.json')
    Given path newPathStatus
    And param apikey = key
