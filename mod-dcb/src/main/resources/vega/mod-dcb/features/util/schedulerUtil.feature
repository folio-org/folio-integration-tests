Feature: scheduler utility for mod-dcb (request-expiration timer)
  ...
  Scenario: find request-expiration timer and update its delay
    ...
    * if (lookupNeeded) karate.call(true, 'classpath:vega/mod-dcb/features/util/schedulerUtil.feature@FindOrCreateRequestExpirationTimer', { extToken: extToken, extUnit: extUnit, extDelay: extDelay })

    # update delay only when timer already existed (created timers already have the desired delay)
    * if (!currentTimerCreated) karate.call(true, 'classpath:vega/mod-dcb/features/util/schedulerUtil.feature@PutRequestExpirationTimer', { extToken: extToken, extTimerId: currentTimerId, extModuleId: currentModuleId, extModuleName: currentModuleName, extUnit: extUnit, extDelay: extDelay })
  ...
  Scenario: find request-expiration scheduler timer, or create it if not registered
    ...
    * if (!timerFound) karate.call(true, 'classpath:vega/mod-dcb/features/util/schedulerUtil.feature@CreateRequestExpirationTimer', { extToken: extToken, extUnit: extUnit, extDelay: extDelay })
