Feature: Borrowing Flow Scenarios
  ...
  Scenario: Create DCB transaction using two shadow locations, update it and verify requests and transaction statuses
    ...
    * def createInst1 = call read('classpath:vega/mod-dcb/reusable/create-institution.feature') { name: 'Shadow Institution 1', code: '#(shadowInstCode1)', isShadow: true }
    * def shadowInstitutionId1 = createInst1.response.id
    * def createCamp1 = call read('classpath:vega/mod-dcb/reusable/create-campus.feature') { name: 'Shadow Campus 1', code: '#(shadowCampCode1)', institutionId: '#(shadowInstitutionId1)', isShadow: true }
    * def shadowCampusId1 = createCamp1.response.id
    * def createLib1 = call read('classpath:vega/mod-dcb/reusable/create-library.feature') { name: 'Shadow Library 1', code: '#(shadowLibCode1)', campusId: '#(shadowCampusId1)', isShadow: true }
    * def shadowLibraryId1 = createLib1.response.id
    * def createLoc1 = call read('classpath:vega/mod-dcb/reusable/create-location.feature') { name: 'Shadow Location 1', code: '#(shadowLocCode1)', institutionId: '#(shadowInstitutionId1)', campusId: '#(shadowCampusId1)', libraryId: '#(shadowLibraryId1)', servicePointId: '#(servicePointId)', isShadow: true }
    ...
    * def createInst2 = call read('classpath:vega/mod-dcb/reusable/create-institution.feature') { name: 'Shadow Institution 2', code: '#(shadowInstCode2)', isShadow: true }
    * def shadowInstitutionId2 = createInst2.response.id
    * def createCamp2 = call read('classpath:vega/mod-dcb/reusable/create-campus.feature') { name: 'Shadow Campus 2', code: '#(shadowCampCode2)', institutionId: '#(shadowInstitutionId2)', isShadow: true }
    * def shadowCampusId2 = createCamp2.response.id
    * def createLib2 = call read('classpath:vega/mod-dcb/reusable/create-library.feature') { name: 'Shadow Library 2', code: '#(shadowLibCode2)', campusId: '#(shadowCampusId2)', isShadow: true }
    * def shadowLibraryId2 = createLib2.response.id
    * def createLoc2 = call read('classpath:vega/mod-dcb/reusable/create-location.feature') { name: 'Shadow Location 2', code: '#(shadowLocCode2)', institutionId: '#(shadowInstitutionId2)', campusId: '#(shadowCampusId2)', libraryId: '#(shadowLibraryId2)', servicePointId: '#(servicePointId)', isShadow: true }
    ...
    * def materialTypeEntityRequest2 = read('classpath:vega/mod-dcb/features/samples/item/material-type-entity-request.json')
    ...
    * def createReq = read('classpath:vega/mod-dcb/features/samples/transaction/create-dcb-transaction.json')
    ...
    * def pollConfig2 = { expectedRecords: 2, path: '#(newPathStatus)', apikey: '#(key)', baseUrl: '#(baseUrlNew)', startDate: '#(startDate)' }
    ...
    Given def pollResult2 = call read("classpath:vega/mod-dcb/reusable/poll-transaction-statuses.feature@PollTransactionStatuses") { config: '#(pollConfig2)' }
