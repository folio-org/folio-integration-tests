Feature: Pickup Flow With Flexible Locations Scenarios
  ...
  Background:
    ...
    * def locationSetup = callonce read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@PostRefreshShadowLocation') { location: '#(dcbLocation)' }
  ...
  Scenario: Create DCB Transaction with location code: LB0
    ...
    * def dcbTransaction = read('classpath:vega/mod-dcb/features/samples/transaction/create-dcb-transaction.json')
  ...
  Scenario: Create DCB Transaction with lending library code: UNKNOWN
    ...
    * def searchArgs = { name: 'DCB', code: '000', isShadow: false }
    Given call read('classpath:vega/mod-dcb/reusable/refresh-shadow-locations.feature@GetLocationByNameAndCode') searchArgs
